...
export default function BookingPage() {
  ...
  if (sent) {
    return (
      <main className="min-h-screen pt-28 pb-20 bg-gradient-to-b from-white via-blue-50/20 to-white">
        <div className="max-w-2xl mx-auto px-4">
          ...
          >
            ...
            <h3 className="text-2xl font-bold font-[Poppins] text-[#0a0a1a] mb-3">Details Sent Successfully!</h3>
    ...
    <main className="min-h-screen pt-28 pb-20 bg-gradient-to-b from-white via-blue-50/20 to-white">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          ...
          <h1 className="text-4xl md:text-5xl font-bold font-[Poppins] text-[#0a0a1a] mt-4 mb-4">
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="relative">
                ...
                <input type="text" name="customerName" value={formData.customerName} onChange={handleChange} onBlur={handleBlur}
                  placeholder="John Doe" className={`input-field ${errors.customerName && touched.customerName ? '!border-red-400 !ring-red-400/20 focus:!ring-red-400/30' : ''}`} required />
                {errors.customerName && touched.customerName && (
                  <p className="flex items-center gap-1.5 mt-1.5 text-xs text-red-500"><AlertCircle size={12} /> {errors.customerName}</p>
                )}
              </div>
              <div className="relative">
                <label className="block text-sm font-medium text-gray-700 mb-2">Phone Number *</label>
                <input type="tel" name="phone" value={formData.phone} onChange={handleChange} onBlur={handleBlur}
                  placeholder="+91 9876543210" className={`input-field ${errors.phone && touched.phone ? '!border-red-400 !ring-red-400/20 focus:!ring-red-400/30' : ''}`} required />
                {errors.phone && touched.phone && (
                  <p className="flex items-center gap-1.5 mt-1.5 text-xs text-red-500"><AlertCircle size={12} /> {errors.phone}</p>
                )}
              </div>
              <div className="relative">
                <label className="block text-sm font-medium text-gray-700 mb-2">Email Address *</label>
                <input type="email" name="email" value={formData.email} onChange={handleChange} onBlur={handleBlur}
                  placeholder="john@example.com" className={`input-field ${errors.email && touched.email ? '!border-red-400 !ring-red-400/20 focus:!ring-red-400/30' : ''}`} required />
                {errors.email && touched.email && (
                  <p className="flex items-center gap-1.5 mt-1.5 text-xs text-red-500"><AlertCircle size={12} /> {errors.email}</p>
                )}
              </div>
              <div className="relative">
                <label className="block text-sm font-medium text-gray-700 mb-2">Business Name *</label>
                <input type="text" name="businessName" value={formData.businessName} onChange={handleChange} onBlur={handleBlur}
                  placeholder="Your Business Name" className={`input-field ${errors.businessName && touched.businessName ? '!border-red-400 !ring-red-400/20 focus:!ring-red-400/30' : ''}`} required />
                {errors.businessName && touched.businessName && (
                  <p className="flex items-center gap-1.5 mt-1.5 text-xs text-red-500"><AlertCircle size={12} /> {errors.businessName}</p>
                )}
              </div>
              <div className="relative">
                ...
                <select name="service" value={formData.service} onChange={handleChange} onBlur={handleBlur}
                  className={`input-field ${errors.service && touched.service ? '!border-red-400 !ring-red-400/20 focus:!ring-red-400/30' : ''}`} required>
                  <option value="">Select a service</option>
                  {services.map(s => <option key={s} value={s}>{s}</option>)}
                </select>
                {errors.service && touched.service && (
                  <p className="flex items-center gap-1.5 mt-1.5 text-xs text-red-500"><AlertCircle size={12} /> {errors.service}</p>
                )}
              </div>
              <div className="relative">
                <label className="block text-sm font-medium text-gray-700 mb-2">Estimated Budget *</label>
                <select name="budget" value={formData.budget} onChange={handleChange} onBlur={handleBlur}
                  className={`input-field ${errors.budget && touched.budget ? '!border-red-400 !ring-red-400/20 focus:!ring-red-400/30' : ''}`} required>
                  <option value="">Select budget range</option>
                  {budgetRanges.map(b => <option key={b} value={b}>{b}</option>)}
                </select>
                {errors.budget && touched.budget && (
                  <p className="flex items-center gap-1.5 mt-1.5 text-xs text-red-500"><AlertCircle size={12} /> {errors.budget}</p>
                )}
              </div>
            </div>
            ...
            <div className="relative">
              ...
              <textarea name="projectDescription" value={formData.projectDescription} onChange={handleChange} onBlur={handleBlur}
                ...
                className={`input-field ${errors.projectDescription && touched.projectDescription ? '!border-red-400 !ring-red-400/20 focus:!ring-red-400/30' : ''}`} rows={5} required />
              {errors.projectDescription && touched.projectDescription && (
                <p className="flex items-center gap-1.5 mt-1.5 text-xs text-red-500"><AlertCircle size={12} /> {errors.projectDescription}</p>
              )}
            </div>
            ...
            <div className="relative">
              ...
              <input type="text" name="preferredDeadline" value={formData.preferredDeadline} onChange={handleChange} onBlur={handleBlur}
                ...
                className={`input-field ${errors.preferredDeadline && touched.preferredDeadline ? '!border-red-400 !ring-red-400/20 focus:!ring-red-400/30' : ''}`} required />
              {errors.preferredDeadline && touched.preferredDeadline && (
                <p className="flex items-center gap-1.5 mt-1.5 text-xs text-red-500"><AlertCircle size={12} /> {errors.preferredDeadline}</p>
              )}
            </div>
